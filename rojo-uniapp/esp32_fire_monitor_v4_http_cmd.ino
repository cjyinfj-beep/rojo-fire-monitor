/*
 * ESP32-S3 消防监测系统 - 完全版（HTTP 命令轮询）
 * 设备ID改为 EX-001（与后端一致）
 * 报警期间用 HTTP 轮询查询命令，不再依赖 MQTT
 */
...
const char* DEVICE_ID = "EX-001";  // 必须与后端设备ID一致
const char* HTTP_SERVER_URL = "http://139.196.153.51/api/mqtt/webhook";  // 数据上报URL
...
const char* TOPIC_SUB_BROADCAST = "device/cmd";
const char* TOPIC_SUB_ID = "device/EX-001/cmd";
const char* TOPIC_SUB_WILDCARD = "device/+/cmd";
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("消防监测系统 - HTTP 命令轮询版");
    ...
    while (millis() - alarmStart < ALARM_KEEP_AWAKE_MS) {
      ...
      if (millis() - lastCmdCheckTime >= 1000) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          if (cmd == "buzzer_off") {
            Serial.println("[HTTP命令] 收到: buzzer_off");
            buzzerContinuousOff();
            continuousOn = false;
            ackCommandHTTP("buzzer_off");  // 确认已执行
            updateDisplay();
            
          } else if (cmd == "buzzer_on") {
            Serial.println("[HTTP命令] 收到: buzzer_on");
            buzzerContinuousOn(2000);
            continuousOn = true;
            ...
            Serial.println("[HTTP命令] 收到: led_on");
            digitalWrite(PIN_LED_RED, HIGH);
            ackCommandHTTP("led_on");
            
          } else if (cmd == "led_off") {
            Serial.println("[HTTP命令] 收到: led_off");
...
String checkCommandHTTP() {
  ...
  if (httpCode == 200) {
    ...
    Serial.print("[HTTP轮询] 响应: ");
    Serial.println(response);
    
    // 解析 JSON: {"has_command": true, "command": "buzzer_off"}
...
bool ackCommandHTTP(const char* command) {
  ...
  if (httpCode == 200) {
    Serial.printf("[HTTP确认] 命令 %s 已确认\n", command);
    http.end();
    return true;
  } else {
    Serial.printf("[HTTP确认] 失败, 码: %d\n", httpCode);
    http.end();
    return false;
  }
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (message.equals("buzzer_off")) {
    Serial.println("[MQTT命令] 执行: 关闭蜂鸣器");
    buzzerContinuousOff();
    updateDisplay();
  } else if (message.equals("buzzer_on")) {
    Serial.println("[MQTT命令] 执行: 开启蜂鸣器");
    buzzerContinuousOn(2000);
    updateDisplay();
  } else if (message.equals("led_on")) {
    Serial.println("[MQTT命令] 执行: 开启红色LED");
    digitalWrite(PIN_LED_RED, HIGH);
  } else if (message.equals("led_off")) {
    Serial.println("[MQTT命令] 执行: 关闭红色LED");
    digitalWrite(PIN_LED_RED, LOW);
  }
  Serial.println("============================================");
}
