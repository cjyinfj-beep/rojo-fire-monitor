/*
 * ESP32-S3 消防监测系统 - v8（修复跨唤醒静音 + 报警解除重置版）
 * 
 * 修复内容：
 * - 报警解除时自动重置用户静音状态，下次新报警可以重新响应
 * - 用 bootCount 代替 millis() 记录静音时间，Deep-Sleep 后仍有效
 * 
 * 核心功能：
 * 1. 用户关闭蜂鸣器/LED 后，报警期间保持，报警解除后重置
 * 2. 蜂鸣器和LED完全独立控制
 * 3. 颜色滤波：连续3次相同颜色才确认
 */
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("消防监测系统 - v8 报警解除重置静音版");
    ...
    while (millis() - alarmStart < ALARM_KEEP_AWAKE_MS) {
      ...
      if (millis() - lastCmdCheckTime >= 1000) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          if (cmd == "buzzer_off") {
            Serial.println("[命令] 用户关闭蜂鸣器 → 本次报警保持静音");
            ...
            Serial.println("[命令] 用户关闭LED → 本次报警保持关闭");
...
// ============================================================================
// 静音超时检查（用 bootCount 代替 millis()，Deep-Sleep 后仍有效）
// ============================================================================
void checkSilenceTimeout() {
