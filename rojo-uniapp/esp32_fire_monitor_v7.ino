/*
 * ESP32-S3 消防监测系统 - v7（RTC 静音保持 + 颜色滤波版）
 * 
 * 核心改进：
 * 1. 用户静音状态存入 RTC 内存，Deep-Sleep 后保持
 *    - 30分钟超时自动取消，避免永久静音
 * 2. 蜂鸣器和LED完全独立控制
 * 3. 颜色滤波：连续3次相同颜色才确认
 */
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("消防监测系统 - v7 RTC静音保持版");
  ...
  if (userSilencedBuzzer) Serial.println("[状态] 蜂鸣器: 用户静音中");
  if (userSilencedLed) Serial.println("[状态] LED: 用户关闭中");
  Serial.println("----------------------------------------\n");
      ...
      if (millis() - lastCmdCheckTime >= 1000) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          if (cmd == "buzzer_off") {
            ...
            silenceTimestamp = millis() / 1000;  // 记录静音时间
            buzzerContinuousOff();
            continuousOn = false;
...
void checkSilenceTimeout() {
  ...
  if (silenceTimestamp > 0 && (now - silenceTimestamp) > SILENCE_TIMEOUT_SEC) {
    Serial.println("[静音超时] 30分钟已到，自动取消静音状态");
    userSilencedBuzzer = false;
    userSilencedLed = false;
    silenceTimestamp = 0;
  } else if (userSilencedBuzzer || userSilencedLed) {
    uint32_t remaining = SILENCE_TIMEOUT_SEC - (now - silenceTimestamp);
    Serial.print("[静音状态] 剩余时间: ");
...
void powerOnPeripherals() {
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0);
    display.println(F("Fire Monitor v7"));
